...
import { detectPackageManager, getBwrapInstallHint, isPathOutsideCwd, looksLikeContainerCommand, truncateCommandForDisplay, } from "./utils.js";
...
export default function (pi) {
    ...
    const registerFailClosedBash = (toolCwd, reason) => {
    ...
    pi.on("session_start", async (_event, ctx) => {
        const noBwrap = pi.getFlag("no-bwrap");
        if (noBwrap) {
            active = false;
            ctx.ui.setStatus("bwrap", ctx.ui.theme.fg("muted", "bwrap: off"));
        ...
        pi.on("tool_call", async (event, toolCtx) => {
            ...
            if (!pathArgName)
                return;
            const targetPath = event.input[pathArgName];
            if (!targetPath)
                return;
            // Allow writes to system temp directory
            if (!isPathOutsideCwd(targetPath, tmpdir()))
                return;
            if (!isPathOutsideCwd(targetPath, toolCtx.cwd))
                return;
            if (!config.promptOnFailure || !toolCtx.hasUI) {
                return { block: true, reason: `Write outside working directory blocked: ${targetPath}` };
            }
            const ok = await toolCtx.ui.confirm("Write outside cwd", `Tool "${event.toolName}" wants to write to:\n${targetPath}\n\nAllow?`);
            if (!ok) {
        ...
        const extendedSchema = Type.Object({
            ...
            unsandboxed: Type.Optional(Type.Boolean({
                description: "If true, run this command outside the sandbox. " +
        ...
        pi.registerTool({
            ...
            description: (bwrapDef.description ?? "") +
            ...
            async execute(toolCallId, params, signal, onUpdate, ctx) {
                ...
                return executeWithFallback(toolCallId, params, signal, onUpdate, ctx, bwrapDef.execute.bind(bwrapDef), localDef.execute.bind(localDef));
            },
        });
        if (!userToggled)
            active = true;
